        ...
        if (err) throw err
            ...
            db.query('SELECT * FROM consumer', function (err, results) {
                if (err) throw err
                    ...
                    db.query('INSERT INTO consumer(consumer_id, consumer_first_name, consumer_last_name, consumer_phone_number, consumer_email, consumer_password, consumer_online_pin, bank_bank_ifsc, credit_points, is_face_recognized, activation) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [c_id, consumer_first_name, consumer_last_name, consumer_phone_number, consumer_email, consumer_password, '0000', 'UNOL0000001', 100, "Y", 0], function (err, results) {
                        if (err) throw err
                        else {
    ...
    if (req.body.report == "trep") {
        ...
        db.query('SELECT * FROM consumer_transaction', function (error, results, fields) {
            if (error) throw error;
                ...
                res.setHeader("Content-Disposition", "attachment; filename=" + "tr_statement.xlsx");
        ...
        db.query('SELECT consumer_id,consumer_first_name,consumer_last_name,consumer_phone_number,consumer_email,is_face_recognized,bank_bank_ifsc,credit_points,activation FROM consumer', function (error, results, fields) {
            if (error) throw error;
                ...
                worksheet.addRows(jsonTransac);
                res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                res.setHeader("Content-Disposition", "attachment; filename=" + "c_statement.xlsx");
                workbook.xlsx.write(res)
                    .then(function (data) {
                        console.log("File saved!");
                        res.end()
        ...
        db.query('SELECT * FROM payee', function (error, results, fields) {
            if (error) throw error;
                ...
                worksheet.addRows(jsonTransac);
                res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                res.setHeader("Content-Disposition", "attachment; filename=" + "py_statement.xlsx");
                workbook.xlsx.write(res)
                    .then(function (data) {
                        console.log("File saved!");
                        res.end()
        ...
        db.query('SELECT * FROM consumer_account', function (error, results, fields) {
            if (error) throw error;
                ...
                worksheet.addRows(jsonTransac);
                res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                res.setHeader("Content-Disposition", "attachment; filename=" + "ca_statement.xlsx");
                workbook.xlsx.write(res)
                    .then(function (data) {
                        console.log("File saved!");
                        res.end()
...
app.post('/sendmessage', function (request, response) {
    ...
    if (username == "root" && password == "root") {
        ...
        db.query('SELECT consumer_id, consumer_phone_number, bank_bank_ifsc, activation FROM consumer where consumer_id = ? and consumer_password = ?', [username, password], function (error, results, fields) {
            if (error) throw error;
            if (results.length > 0 && results[0].activation==1) {
                ...
                //response.redirect('/home/getdata')
                generateOTP()
    ...
    if (req.body.etype == "all") {
        db.query('SELECT * FROM consumer_transaction where account_from_account_number = ?', [req.body.acc], function (error, results, fields) {
            if (error) throw error;
                ...
                worksheet.addRows(jsonTransac);
                res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                res.setHeader("Content-Disposition", "attachment; filename=" + "e-statement.xlsx");
                workbook.xlsx.write(res)
                    .then(function (data) {
                        console.log("File saved!");
                        res.end()
...
function generateOTP() {
    ...
    db.query('SELECT * FROM otp where consumer_id=?', [cid], function (error, results, fields) {
        if (error) throw error;
        if (results.length > 0) {
            db.query('UPDATE otp SET otp = ?, sent =? where consumer_id=?', ["0000", timestamp1.toString(), cid], function (error, results, fields) {
                if (error) throw error;
                else { console.log("Updated") }
            })
        }
        else {
            db.query('INSERT INTO otp (consumer_id, otp, sent) VALUES (?,?,?)', [cid, "0000", timestamp1.toString()], function (error, results, fields) {
                if (error) throw error;
